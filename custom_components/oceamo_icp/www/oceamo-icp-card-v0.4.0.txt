...
class OceamoIcpHistoryCard extends HTMLElement {
  ...
  async _load() {
    ...
    try {
      ...
      console.warn("Oceamo ICP history load failed", error);
      if (requestId !== this._requestId) return;
    ...
    const circles = points
      .map((point, index) => {
        ...
        const title = `${historyDateLabel(
          point.start,
          this._language
        )}${analysis} · ${this._summaryValue(point.value)}`;
  ...
  _pointDetail(index) {
    ...
    return `
      ...
      ${
        analysis
          ? `<strong>${escapeHtml(analysis)}</strong>`
          : ""
      }
...
if (!window.customCards.some((card) => card.type === CARD_TAG)) {
  window.customCards.push({
    ...
    description:
      "Displays Oceamo ICP analyses with targets, status, comparison and interactive measurement history.",
    preview: true,
