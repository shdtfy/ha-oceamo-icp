...
function actionPlanSignalText(signal, t) {
  ...
  if (signal.source === "stocking_profile" && signal.current_issue) {
                              ...
                              actions.length
                                ? `
                                  <div class="action-source-list">
                                    ${actions
                                      .map((action) => {
                                        const text = actionPlanActionText(
                                          action,
                                          language,
                                          t
                                        );
                                        const splitText =
                                          ...
                                          action.action === "correction_dose" &&
                                          action.split_days > 1 &&
                                          typeof action.daily_dose_amount ===
                                            "number"
                                            ? interpolation(
                                                t.actionCorrectionSplit,
                                                {
                              ...
                              signals.length
                                ? `
                                  <div class="action-signal-list">
                                    ${signals
                                      .map((signal) => {
                                        const text = actionPlanSignalText(
                                          signal,
                                          t
                                        );
                                        if (!text) return "";
